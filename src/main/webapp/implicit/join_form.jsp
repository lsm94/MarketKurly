...
<%-- => [가입]을 클릭한 경우 form 태그로 처리페이지(join_action.jsp)를 요청(POST)하여 페이지 이동 - 입력값 전달 --%>
<%
	/*
	//전달값(에러메세지)을 반환받아 저장
	...
	*/
	...
	//session.getAttribute(String name) : JSP 문서에 바인딩된 세션(HttpSession)에 저장된
	//세션 속성값을 세션 속성명으로 반환하는 메소드 - 세션 속성값이 없는 경우 null 반환
	// => Object 객체로 반환되므로 명시적 객체 형변환하여 사용
	String message=(String)session.getAttribute("message");
	if(message==null) {
		message="";
	} else {
		//session.removeAttribute(String name) : JSP 문서에 바인딩된 세션(HttpSession)에 저장된
		//세션 속성값을 세션 속성명으로 삭제하는 메소드
...
<h3 align="center">◆◆◆ 회원가입 ◆◆◆</h3>
	...
	<%-- 표현식(Expression)의 변수에 null이 저장된 경우 [null] 문자열로 변환되어 출력 --%>
