...
<%-- => [학생추가]를 클릭한 경우 학생정보 입력페이지(insertFormStudent.jsp)를 요청하여 이동 --%>    
<%-- => [삭제]를 클릭한 경우 학생정보 삭제페이지(deleteStudent.jsp)를 요청하고 학번 전달하여 이동 --%>    
<%-- => [변경]를 클릭한 경우 학생정보 입력페이지(updateFormStudent.jsp)를 요청하고 학번 전달하여 이동 --%>
<%
...
<body>
	...
	<table align="center" border="1" cellspacing="0" cellpadding="1" width="800">
		...
		<% } else {//List 객체에 저장된 요소가 있는 경우 %>
			<%-- List 객체에 저장된 요소를 차례대로 제공받아 응답 처리되도록 반복 --%>
			<%-- => 반복문에서는 요소(학생정보 - StudentDTO 객체)의 필드값을 클라이언트에게 전달 --%>
